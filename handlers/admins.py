...
async def update_admin(client, message):
    ...
    await client.send_message(message.chat.id, "✅ Bot **Dimulai ulang !**\n\n• **Daftar admin** Telah **Diperbarui !**""",
                 reply_markup=InlineKeyboardMarkup(
                     [
                         [
                             InlineKeyboardButton(
                                 "Channel 💬", url=f"https://t.me/Rizzz_Support"
                             ),
                             InlineKeyboardButton(
...
async def pause(_, message: Message):
    ...
    if (chat_id not in callsmusic.pytgcalls.active_calls) or (
        ...
        await message.reply_text("▶️ **Music dijeda!**\n\n• To resume music use **command » /resume**")
...
async def resume(_, message: Message):
    ...
    if (chat_id not in callsmusic.pytgcalls.active_calls) or (
        ...
        await message.reply_text("❌ **Tidak ada music yang dijeda!**")
    else:
...
async def stop(_, message: Message):
    ...
    if chat_id not in callsmusic.pytgcalls.active_calls:
        await message.reply_text("❌ **Tidak sedang memainkan lagu!**")
    else:
        try:
...
async def skip(_, message: Message):
    ...
    if not qeue:
        return
    await message.reply_text(f"⏭️ **__Anda telah melompat ke lagu berikutnya__**")
